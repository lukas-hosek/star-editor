...
import { project, unproject, pixelToNDC, dirToRADec } from './camera.js';
...
export function pickStar(
	...
	for (let i = 0; i < stars.length; i++) {
		...
		const { dir, flux } = displayedDirAndFlux(s, dtYears);
		if (flux * brightness <= minScreenBrightness) continue;
		const { nx: snx, ny: sny, zc } = project(cam, dir);
	...
	if (bestI !== -1) {
		...
		if (picked.primaryHygId !== null && picked.primaryHygId !== picked.hygId) {
			for (let i = 0; i < stars.length; i++) {
				...
				const pv = displayedDirAndFlux(stars[i], dtYears).dir;
				const { nx: pnx, ny: pny, zc: pzc } = project(cam, pv);
				if (pzc < -0.5 || !isFinite(pnx) || !isFinite(pny)) break;
				const sv = displayedDirAndFlux(picked, dtYears).dir;
