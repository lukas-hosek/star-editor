...
{
	const projection = project(camera, point);
	if (!Number.isFinite(projection.nx) || !Number.isFinite(projection.ny) || projection.zc < -0.18) return null;
	const [px, py] = ndcToPixel(camera, projection.nx, projection.ny);
	const pad = 96;
...
function drawRADecLabels(renderer, camera)
{
...
function drawAltAzLabels(renderer, camera, basis)
{
	const ctx = renderer.overlayCtx;
	...
	{
		const pixel = projectWorldPoint(camera, altAzDir(0, label.az, renderer.zenith, basis));
		if (!pixel) continue;
...
function drawRADecGridOverlay(renderer, camera)
{
	if (!renderer.raDecGridVisible) return;
	...
	drawRADecLabels(renderer, camera);
}


function drawAltAzGridOverlay(renderer, camera)
{
	if (!renderer.altAzGridVisible || !renderer.zenith) return;

	const ctx = renderer.overlayCtx;
	const basis = localHorizonBasis(renderer.zenith);

	ctx.lineWidth = 1;
	ctx.strokeStyle = GRID_LINE_COLOR;
	for (let az = 0; az < 2 * Math.PI - ALTAZ_AZ_STEP * 0.5; az += ALTAZ_AZ_STEP)
	{
		strokeProjectedCurve(ctx, camera, (alt) => altAzDir(alt, az, renderer.zenith, basis), -Math.PI / 2, Math.PI / 2, GRID_SAMPLE_STEP);
	}
	...
	for (let alt = -60 * DEG; alt <= 60 * DEG + ALTAZ_ALT_STEP * 0.5; alt += ALTAZ_ALT_STEP)
	{
		...
		strokeProjectedCurve(ctx, camera, (az) => altAzDir(alt, az, renderer.zenith, basis), 0, 2 * Math.PI, GRID_SAMPLE_STEP);
	}

	ctx.strokeStyle = GRID_LINE_COLOR;
	drawAltAzLabels(renderer, camera, basis);
}


export function drawGridOverlay(renderer, camera)
{
	const ctx = renderer.overlayCtx;
	ctx.setTransform(renderer.overlayScale, 0, 0, renderer.overlayScale, 0, 0);
	ctx.clearRect(0, 0, camera.width, camera.height);
	if (!renderer.raDecGridVisible && !renderer.altAzGridVisible) return;

	drawRADecGridOverlay(renderer, camera);
	drawAltAzGridOverlay(renderer, camera);
}