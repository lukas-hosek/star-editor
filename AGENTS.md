...
- A "Time Travel" toolbar button toggles a year-offset mode. When active, a bar (`#time-travel-bar`) appears bottom-center over the sky canvas with a range slider (−100000 to +100000 years from J2000), a year readout, and a `↺` reset button. The bar spans 80 vw (with `box-sizing: border-box` so padding is included) and the slider stretches to 100% of that width.
...
- New per-star GPU attributes (locations 4–6, bound in `STAR_ATTRIB_BINDINGS` in `js/renderer-pipeline.js`): `aPosPc` (vec3, HYG `x/y/z` in parsecs), `aVelPcYr` (vec3, HYG `vx/vy/vz` in parsecs/year), `aAbsMag` (float, HYG `absmag` with `999.0` sentinel when absent).
- The shader propagates `newPos = aPosPc + aVelPcYr * uTimeTravelYears` (HYG velocities are pc/yr, so the uniform is just the year offset). It normalizes with a y-flip (HYG cartesian → `sphereDir` convention) and recomputes flux from `absmag` when `aAbsMag < 50.0` (sentinel check).
- **No validity check for kinematics.** BSC stars (without HYG `x/y/z/vx/vy/vz`) propagate from raw catalog values — `null` → `0` via `Float32Array` coercion → degenerate direction. BSC stars are expected to misbehave during animation. At `uTimeTravelYears == 0` everything still renders correctly because the catalog `aPos` path isn't bypassed in the CPU mirror (picker / selection ring) and the shader's propagation collapses to the same direction for valid kinematic stars. Hand-added stars do carry placeholder kinematics (see `makeNewStar` below), so they behave correctly in time-travel mode — they appear at 20 pc with zero velocity and do not drift.
- Catalog read: `js/catalog-hyg.js` now parses HYG column 14 (`absmag`) and the serializer writes it back. BSC stars have `absmag = null`. Newly-added stars get `absmag = Vmag - 5·log10(dist/10)` (= `−5·log10(2)` ≈ −1.505 for the default Vmag=0 at 20 pc).
...
- **Bake** button (`#btn-time-travel-bake`) on the time-travel bar commits the propagated display back to the catalog: for every star with HYG kinematics it rewrites `ra`/`dec`/`x`/`y`/`z`/`Vmag`/`Parallax`/`pmRA`/`pmDE` to the time-traveled values, then disables time-travel mode. Stars without kinematics (BSC, hand-added) are skipped. Implementation: `bakeStar(star, dtYears)` in `js/time-travel.js` (in-place; returns whether the star was baked) → `bakeTimeTravel()` action in `js/app-editor-actions.js` iterates stars, runs `refreshStarPhotometry` on each baked star, calls `syncAll`, marks `state.isDirty`, refreshes the sidebar for the selected star, and calls `setTimeTravelEnabled(false)`. Velocities (`vx/vy/vz`) and `absmag` are intrinsic to the star and intentionally untouched; `pmRA`/`pmDE` are recomputed from kinematics in HYG's great-circle convention (`μ_α · cos δ`).
