...
- **No validity check for kinematics.** Stars without HYG `x/y/z/vx/vy/vz` (BSC stars, hand-added stars) propagate from raw catalog values — `null` → `0` via `Float32Array` coercion → degenerate direction. The time-travel feature is HYG-only; BSC stars are expected to misbehave during animation. At `uTimeTravelYears == 0` everything still renders correctly because the catalog `aPos` path isn't bypassed in the CPU mirror (picker / selection ring) and the shader's propagation collapses to the same direction for valid kinematic stars.
